...
class motion_executioner(Node):
    def __init__(self, motion_type=0):
        ...
        self.imu_subscription = self.create_subscription(
            Imu, "/imu", self.imu_callback, qos_profile=qos
        )
        self.imu_initialized = True

        # ENCODER subscription /odom
        self.odom_subscription = self.create_subscription(
            Odometry, "/odom", self.odom_callback, qos_profile=qos
        )
    ...
    def odom_callback(self, odom_msg: Odometry):
        ...
        msg = [odom_msg.pose.pose.position.x, odom_msg.pose.pose.position.y, th, stamp]
    ...
    def laser_callback(self, laser_msg: LaserScan):
        ...
        valid_ranges = [
            rng if laser_msg.range_min < rng < laser_msg.range_max else np.NaN
            for rng in laser_msg.ranges
        ]

        stamp = laser_msg.header.stamp.sec * 1e9 + laser_msg.header.stamp.nanosec

        self.laser_logger.log_values([valid_ranges, stamp])
