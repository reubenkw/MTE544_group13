...
def plot_errors(filename: str):
    ...
    plt.savefig(
        f"plots/{path_type.lower()}_{sensor.lower()}.png",
        transparent=True,
        bbox_inches="tight",
    )
...
def plot_laser(filename):
    ...
    plt.savefig(
        f"plots/{path_type.lower()}_lidar_3d.png", transparent=True, bbox_inches="tight"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=False,
        help="List of files to process. If not given, searches \\data dir.",
    )
