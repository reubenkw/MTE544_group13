...
class decision_maker(Node):
    def __init__(
        ...
        if motion_type == POINT_PLANNER:
            self.controller = controller(
                klp=0.8, klv=0.5, kap=0.8, kav=0.6, is_sim=is_sim
            )
            self.planner = planner(POINT_PLANNER)

        elif motion_type == TRAJECTORY_PLANNER:
            self.controller = trajectoryController(
                klp=2.0, klv=1.0, kli=1.0, kap=1.1, kav=0.15, kai=1.0, is_sim=is_sim
            )
    ...
    def timerCallback(self):
        ...
        if isinstance(self.goal, list):
            ...
            reached_goal = (
                calculate_linear_error(crnt_pose, self.goal[-1]) < self.th_linear
            )
        else:
...
def main(args=None):
    ...
    """
    qos_reliability = (
        ReliabilityPolicy.RELIABLE if args.is_sim else ReliabilityPolicy.BEST_EFFORT
    )
    odom_qos = QoSProfile(
